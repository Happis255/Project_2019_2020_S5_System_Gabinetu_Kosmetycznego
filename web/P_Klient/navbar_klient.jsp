<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<!doctype html>
<html>
<head>
    <meta charset="utf-8">
</head>

<body>
<nav class="navbar navbar-dark navbar-expand-md" style="background-color:#2d2d2d;color:rgb(255,255,255);font-weight:100;text-transform:uppercase;margin:0;margin-bottom:30px;">
    <div class="container"><a class="navbar-brand" href="../index.jsp"><img src="../assets/img/LOGO.png"></a><button class="navbar-toggler" data-toggle="collapse" data-target="#navcol-1"><span class="sr-only">Toggle navigation</span></button>
        <div class="collapse navbar-collapse"
             id="navcol-1">
            <ul class="nav navbar-nav ml-auto">
                <li class="nav-item" role="presentation"><a class="nav-link" href="../ControllerAccount?page=wizytyService" style="color:#ffffff;">Utwórz wizytę</a></li>
                <li class="nav-item" role="presentation"><a class="nav-link" href="../ControllerAccount?page=wizyty" style="color:#ffffff;">Twoje wizyty</a></li>
                <li class="nav-item" role="presentation"><a class="nav-link" href="../P_User/oferta_zakupowa.jsp" style="color:#ffffff;">Sklep</a></li>
                <li class="nav-item" role="presentation"><a class="nav-link" href="../ControllerAccount?page=twojebonusy" style="color:#ffffff;">Twoje bonusy</a></li>
                <li class="nav-item" role="presentation"><a class="nav-link" href="../ControllerAccount?page=konto" style="color:#ffffff;">konto</a></li>
                <li class="nav-item" role="presentation"><a class="nav-link" href="../UserLogin" style="color:#ffffff;">wyloguj</a></li>
            </ul>
        </div>
    </div>
</nav>
</body>
</html>
